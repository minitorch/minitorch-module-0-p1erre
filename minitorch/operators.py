...
def is_close(x: float, y: float, tol: float = 1e-2) -> bool:
    ...
    return x != None and y != None and abs(x-y) < tol
...
# - zipWith
def zipWith(fn: Callable[[Any, Any,], Any], xs: Iterable[Any], ys: Iterable[Any]) -> Iterable[Any]:
...
def reduce(fn: Callable[[Any, Any], Any], xs: Iterable[Any]) -> Any:
    if not xs: return None # Should raise an exception here?
    ixs = iter(xs)
    x = next(ixs)
    for y in ixs:
        x = fn(x, y)
